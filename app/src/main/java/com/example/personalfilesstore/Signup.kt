    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        btnSignup.setOnClickListener {
            if (email.text.toString().equals("") || password.text.toString()
                    .equals("") || name.text.toString().equals("")
            ) {
        ...
        FirebaseAuth.getInstance().createUserWithEmailAndPassword(email, password)
            .addOnCompleteListener {
                if (it.isSuccessful) {
                    ...
                    FirebaseDatabase.getInstance().reference.child(it.result.user!!.uid.toString())
                        .setValue(user).addOnCompleteListener {
                        Toast.makeText(this@Signup, "Registration successful", Toast.LENGTH_LONG)
                            .show()
                            sendEmailVerification()
                    }
                } else {
                    Toast.makeText(this@Signup, "Error encountered try again", Toast.LENGTH_LONG)
                        .show()
                }
            }
    }
        ...
        firebaseUser?.let {
            it.sendEmailVerification().addOnCompleteListener { task ->
                if (task.isSuccessful) {
                    Toast.makeText(this@Signup, "email sent to your email", Toast.LENGTH_LONG)
                        .show()
                }
            }
        }
